export default{
    template : `<header>
                <h2>간단한 게시판</h2>
                <p>게시판 데이터 json 파일 읽기</p>
                <input type="file" v-on:change="loadData($event)">
                <button v-on:click="saveBoardList">게시판 저장하기</button>
                </header>`,
    props : ['parentData'],
    methods : {

        loadData : function(event){ // 파일을 읽어들이는 메소드
                        
            let fileName = event.target.files[0].name; // event가 발생한 객체 (target).files[n]        

            if(fileName) {  

            fetch('/web/board02/data/'+fileName) // board02를 route로 만들었기 때문에 상대경로 사용불가
                                                 // VS Code 기준 최상위폴더를 보세요.
            .then(response => response.json())
            .then(data => {
                this.parentData.dataArray = data;
                if(this.parentData.dataArray !=null && this.parentData.dataArray['board'].length > 0){
                    this.parentData.listOk =true;
                }
                this.$emit('update:parentData', this.parentData) // 어느 값을 update할 지 가져오고, 값을 주기

                this.$router.push({name:'boardList'}); // 단순 페이지 이동

            })
            .catch(err => console.log(err));
            }  
        },

        saveBoardList : function(){ // 파일 생성 (게시판 저장)
            // 이건 어려워서 혼자 다룰 수 없음..

            let data = this.parentData.dataArray;

            if(data.length == 0){ // 존재하지 않는 값을 처리할 순 없으니
                alert('저장할 게시판 내용이 없습니다.');
                return;
            }

            if(typeof data === 'object') { // === type과 내용까지 일치여부
                data = JSON.stringify(data, undefined, 4);
            }

            let blob = new Blob([data],{type:'text/json'});
            let a = document.createElement('a');
            a.download='board.json';
            a.href = window.URL.createObjectURL(blob);
            a.click();
        }
    }
}